...
func main() {
	fmt.Println("Hello from Uaa Token Manager")

	var savedTokenStores = LoadSavedTokenStores()
	//savedTokenStores =
	//fmt.Println(savedTokenStores)
	...
	if *addPtr {
		...
		// var ts = TokenStore{
		// 	uaaURL:       "https://edgemanager-cf3-int.predix-uaa.run.aws-usw02-dev.ice.predix.io",
		// 	clientID:     "default-app-client",
		// 	clientSecret: "-UKGjnEqVN06jNr",
		// }
		var ts = TokenStore{
			UaaURL:       uaaUrl,
			ClientID:     cid,
	...
	if len(savedTokenStores) > 0 {
		...
		fmt.Println("No Tokens are currently Managed. Use the -add option to add new tokens")
	}
